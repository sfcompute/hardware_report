...
pub fn parse_size_to_bytes(size_str: &str) -> Result<u64, String> {
    ...
    if let Some(start) = size_str.find('(') {
        let end_pos = size_str.find(" Bytes)").or_else(|| size_str.find(" bytes)"));
...
pub fn parse_key_value(line: &str, separator: char) -> Result<(String, String), String> {
    if let Some(pos) = line.find(separator) {
        ...
        Err(format!("No separator '{}' found in line: {}", separator, line))
    }
}
...
mod tests {
    ...
    fn test_parse_size_to_bytes() {
        assert_eq!(parse_size_to_bytes("16GB").unwrap(), 16 * 1024 * 1024 * 1024);
        assert_eq!(parse_size_to_bytes("2.5TB").unwrap(), (2.5 * 1024.0 * 1024.0 * 1024.0 * 1024.0) as u64);
    ...
    fn test_extract_dmidecode_value() {
        ...
        assert_eq!(extract_dmidecode_value(output, "Manufacturer").unwrap(), "Dell Inc.");
        assert_eq!(extract_dmidecode_value(output, "Product Name").unwrap(), "PowerEdge R740");
