...
use crate::domain::{HardwareReport, PublishConfig, ReportConfig, ReportError, PublishError};
...
pub trait HardwareMonitoringService: Send + Sync {
    ...
    async fn start_monitoring(&self, interval_seconds: u64, config: ReportConfig) -> Result<MonitoringHandle, ReportError>;
}
