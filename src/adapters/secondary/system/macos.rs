...
use crate::domain::{
    ...
    parse_macos_cpu_info, parse_macos_memory_info, parse_macos_storage_info,
    parse_macos_network_info, parse_hostname_output
};
...
impl MacOSSystemInfoProvider {
    ...
    pub async fn check_required_commands(&self) -> Vec<String> {
        let required_commands = [
            "system_profiler", "sysctl", "ioreg", "hostname", "df"
        ];
...
impl SystemInfoProvider for MacOSSystemInfoProvider {
    async fn get_cpu_info(&self) -> Result<CpuInfo, SystemError> {
        ...
        let output = self.command_executor.execute(&system_profiler_cmd).await
        ...
        parse_macos_cpu_info(&output.stdout)
    ...
    async fn get_memory_info(&self) -> Result<MemoryInfo, SystemError> {
        ...
        parse_macos_memory_info(&output.stdout)
            .map_err(|e| SystemError::ParseError(e))
    }
    ...
    async fn get_storage_info(&self) -> Result<StorageInfo, SystemError> {
        ...
        let devices = parse_macos_storage_info(&output.stdout)
            .map_err(|e| SystemError::ParseError(e))?;
    ...
    async fn get_network_info(&self) -> Result<NetworkInfo, SystemError> {
        let ifconfig_cmd = SystemCommand::new("ifconfig")
            .timeout(Duration::from_secs(10));
        let output = self.command_executor.execute(&ifconfig_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        let interfaces = parse_macos_network_info(&output.stdout)
            .map_err(|e| SystemError::ParseError(e))?;
    ...
    async fn get_bios_info(&self) -> Result<BiosInfo, SystemError> {
        ...
        for line in output.stdout.lines() {
            ...
            if trimmed.starts_with("System Firmware Version:") {
                version = trimmed.split(':').nth(1).unwrap_or("Unknown").trim().to_string();
            } else if trimmed.starts_with("OS Loader Version:") {
                release_date = trimmed.split(':').nth(1).unwrap_or("Unknown").trim().to_string();
            }
        }
    ...
    async fn get_chassis_info(&self) -> Result<ChassisInfo, SystemError> {
        ...
        for line in output.stdout.lines() {
            ...
            if trimmed.starts_with("Model Name:") {
                ...
                if model.contains("iMac") || model.contains("Mac Pro") || model.contains("Mac Studio") {
