...
use crate::domain::{
    CpuInfo, MemoryInfo, StorageInfo, GpuInfo, NetworkInfo, BiosInfo, 
    ChassisInfo, MotherboardInfo, SystemInfo, NumaNode, SystemError,
    parse_lscpu_output, parse_dmidecode_cpu, combine_cpu_info,
    parse_free_output, parse_dmidecode_memory, determine_memory_type, determine_memory_speed,
    parse_lsblk_output, parse_ip_output, parse_dmidecode_system_info,
    parse_dmidecode_bios_info, parse_dmidecode_chassis_info, parse_hostname_output
};
use crate::ports::{SystemInfoProvider, CommandExecutor, SystemCommand};
...
impl LinuxSystemInfoProvider {
    ...
    pub async fn check_required_commands(&self) -> Vec<String> {
        let required_commands = [
            "lscpu", "dmidecode", "free", "lsblk", "ip", "hostname", "df"
        ];
...
impl SystemInfoProvider for LinuxSystemInfoProvider {
    async fn get_cpu_info(&self) -> Result<CpuInfo, SystemError> {
        ...
        let lscpu_cmd = SystemCommand::new("lscpu")
            .timeout(Duration::from_secs(10));
        let lscpu_output = self.command_executor.execute(&lscpu_cmd).await
        ...
        let lscpu_info = parse_lscpu_output(&lscpu_output.stdout)
        ...
        match self.command_executor.execute_with_privileges(&dmidecode_cmd).await {
    ...
    async fn get_memory_info(&self) -> Result<MemoryInfo, SystemError> {
        ...
        let free_output = self.command_executor.execute(&free_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        let total_memory = parse_free_output(&free_output.stdout)
            .map_err(|e| SystemError::ParseError(e))?;
        ...
        let (modules, type_, speed) = match self.command_executor.execute_with_privileges(&dmidecode_cmd).await {
            Ok(dmidecode_output) if dmidecode_output.success => {
    ...
    async fn get_storage_info(&self) -> Result<StorageInfo, SystemError> {
        let lsblk_cmd = SystemCommand::new("lsblk")
            ...
            .timeout(Duration::from_secs(10));
        let lsblk_output = self.command_executor.execute(&lsblk_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        let devices = parse_lsblk_output(&lsblk_output.stdout)
            .map_err(|e| SystemError::ParseError(e))?;
    ...
    async fn get_network_info(&self) -> Result<NetworkInfo, SystemError> {
        let ip_cmd = SystemCommand::new("ip")
            .args(&["addr", "show"])
            .timeout(Duration::from_secs(5));
        let ip_output = self.command_executor.execute(&ip_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        let interfaces = parse_ip_output(&ip_output.stdout)
            .map_err(|e| SystemError::ParseError(e))?;
    ...
    async fn get_bios_info(&self) -> Result<BiosInfo, SystemError> {
        let dmidecode_cmd = SystemCommand::new("dmidecode")
            .args(&["-t", "bios"])
            .timeout(Duration::from_secs(10));
        let dmidecode_output = self.command_executor.execute_with_privileges(&dmidecode_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        parse_dmidecode_bios_info(&dmidecode_output.stdout)
            .map_err(|e| SystemError::ParseError(e))
    }
    ...
    async fn get_hostname(&self) -> Result<String, SystemError> {
        let hostname_cmd = SystemCommand::new("hostname")
            .timeout(Duration::from_secs(5));
        let hostname_output = self.command_executor.execute(&hostname_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        parse_hostname_output(&hostname_output.stdout)
            .map_err(|e| SystemError::ParseError(e))
    }
    ...
    async fn get_filesystems(&self) -> Result<Vec<String>, SystemError> {
        let df_cmd = SystemCommand::new("df")
            ...
            .timeout(Duration::from_secs(5));
        let df_output = self.command_executor.execute(&df_cmd).await
            .map_err(|e| SystemError::CommandFailed {
        ...
        for line in df_output.stdout.lines().skip(1) { // Skip header
    ...
    async fn has_required_privileges(&self) -> Result<bool, SystemError> {
        self.command_executor.has_elevated_privileges().await
            .map_err(|e| SystemError::CommandFailed {
