...
use crate::ports::{CommandExecutor, SystemCommand, CommandOutput};
...
impl UnixCommandExecutor {
    ...
    async fn execute_with_retry(&self, command: &SystemCommand, use_sudo: bool) -> Result<CommandOutput, CommandError> {
    ...
    async fn execute_once(&self, command: &SystemCommand, use_sudo: bool) -> Result<CommandOutput, CommandError> {
        ...
        match result {
            ...
            Ok(Err(e)) => {
                Err(CommandError::ExecutionFailed(format!("Failed to execute command '{}': {}", command.program, e)))
            }
            Err(_) => {
                Err(CommandError::ExecutionFailed(format!("Command '{}' timed out after {:?}", command.program, command_timeout)))
            }
        }
    }
}
...
impl CommandExecutor for UnixCommandExecutor {
    ...
    async fn execute_with_privileges(&self, command: &SystemCommand) -> Result<CommandOutput, CommandError> {
...
mod tests {
    ...
    async fn test_unix_command_executor_basic() {
        ...
        let cmd = SystemCommand::new("echo")
            .args(&["hello", "world"]);
