...
use crate::domain::{ReportConfig, HardwareCollectionService};
use crate::ports::{HardwareReportingService, SystemInfoProvider, CommandExecutor, DataPublisher, ConfigurationProvider};
use crate::adapters::{
    UnixCommandExecutor, LinuxSystemInfoProvider, MacOSSystemInfoProvider, 
    HttpDataPublisher, FileDataPublisher
};
use std::sync::Arc;
...
impl ConfigurationProvider for SimpleConfigurationProvider {
    ...
    async fn get_publish_config(&self) -> Result<Option<crate::domain::PublishConfig>, crate::domain::DomainError> {
        Ok(None) // No publishing by default
    }
    
    async fn get_output_format(&self) -> Result<crate::ports::OutputFormat, crate::domain::DomainError> {
    ...
    async fn get_labels(&self) -> Result<std::collections::HashMap<String, String>, crate::domain::DomainError> {
...
impl ServiceContainer {
    ...
    pub fn create_system_info_provider(&self) -> Result<Arc<dyn SystemInfoProvider>, Box<dyn Error>> {
    ...
    pub fn create_data_publisher(&self) -> Result<Arc<dyn DataPublisher>, Box<dyn Error>> {
        let http_publisher = HttpDataPublisher::new(
    ...
    pub fn create_configuration_provider(&self, report_config: ReportConfig) -> Arc<dyn ConfigurationProvider> {
        Arc::new(SimpleConfigurationProvider::new(report_config))
    }
    
    /// Create the complete hardware reporting service
    pub fn create_hardware_reporting_service(&self, report_config: Option<ReportConfig>) -> Result<Arc<dyn HardwareReportingService>, Box<dyn Error>> {
        let system_provider = self.create_system_info_provider()?;
        let data_publisher = self.create_data_publisher()?;
        let config_provider = self.create_configuration_provider(
            report_config.unwrap_or_default()
        );
        
        let service = HardwareCollectionService::new(
            system_provider,
            data_publisher,
            config_provider,
        );
    ...
    pub async fn validate_dependencies(&self) -> Result<Vec<String>, Box<dyn Error>> {
        let system_provider = self.create_system_info_provider()?;
        let missing = system_provider.get_missing_dependencies().await
    ...
    pub async fn check_privileges(&self) -> Result<bool, Box<dyn Error>> {
        let system_provider = self.create_system_info_provider()?;
        let has_privileges = system_provider.has_required_privileges().await
...
mod tests {
    ...
    fn test_container_creation() {
        ...
        assert_eq!(container.get_platform_name(), if cfg!(target_os = "macos") { "macOS" } else { "Linux" });
    }
