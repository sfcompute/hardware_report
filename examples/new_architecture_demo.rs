...
use hardware_report::{
    create_service, validate_system, ReportConfig, ContainerConfigBuilder,
    create_service_with_config, ServerInfo, FileSystemRepository, FileRepository,
    new_domain::HardwareReport
};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("✅ Legacy architecture report generated in {:?}", legacy_duration);
    ...
    println!("   Hostname match: {} ({})", if hostname_match { "✅" } else { "❌" }, new_report.hostname);
    ...
    let custom_service = create_service_with_config(custom_container_config, Some(custom_report_config)).await?;
    ...
    file_repo.save_json(&new_report, Path::new(&json_path)).await?;
    file_repo.save_toml(&new_report, Path::new(&toml_path)).await?;
    ...
    println!("   JSON round-trip: {}", if json_match { "✅" } else { "❌" });
    println!("   TOML round-trip: {}", if toml_match { "✅" } else { "❌" });
    ...
    println!("✅ Platform Abstraction: Works on {} without code changes", 
